// this file defines authprovider for the rest of the frontend authentication.
...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const restoreRequestId = useRef(0)    // prevents race conditions/overwrites when multiple /me
  ...
  const retrySession = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (problem.status === 401) {   // user logged out or session expired, so set state to unauthenticated
