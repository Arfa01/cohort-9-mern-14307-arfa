// this file defines the register page, with form validation, error handling, and redirect after register
...
export function RegisterPage() {
  const { register: createAccount } = useAuth()       // auth function is named register, but we rename it to createAccount to avoid confusion with react-hook-form's register function
  ...
  const {
    ...
    resolver: zodResolver(registerSchema), // validate all form values using zod schema
    defaultValues: {
      name: '',
      email: '',
      password: '',
  ...
  const handleRegister = async (values: RegisterFormValues) => {
    try {
      ...
      if (problem.code === 'EMAIL_ALREADY_REGISTERED') {   // duplicate email error from backend, map to email field
